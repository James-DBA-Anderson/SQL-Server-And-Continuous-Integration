...
try{
    ...
    Describe -Tag $InstanceLabel ("SQL Server tests on " + $InstanceLabel + " via the " + $Username + " account.") {
        Context "SQL Service" {
            ...
            It "DBEngine should be online" {
                $srv.Status | Should Be Online
            }
            It "Service Start Mode should be auto or manual if clustered" {
                if($srv.IsClustered -eq $true) {
                    $srv.ServiceStartMode | Should Be "manual"
                } else {

                    $srv.ServiceStartMode | Should Be "auto"
                }
            }
